...
public class SimpleDirectoryWatchService implements DirectoryWatchService, Runnable {
	 ...
	 * <code>SimpleDirectoryWatchService</code>.
	 *
	 * @throws IOException
	...
	private Set<OnFileChangeListener> matchedListeners(Path dir, Path file) {
		return getListeners(dir).stream().filter(listener -> matchesAny(file, getPatterns(listener)))
	...
	/**
	 * Start this <code>SimpleDirectoryWatchService</code> instance by spawning a
	 * new thread.
	 *
	 * @see #stop()
	 */
	 ...
	 * happens lazily, giving the running thread an opportunity to finish the work
	 * at hand.
	 *
	 * @see #start()
	 */
